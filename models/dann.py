...
class DANN_Lightning(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        X, y = batch
        label_output, domain_output = self.model(X)
        label_loss = self.criterion(label_output.squeeze(), y.float())
        domain_loss = self.criterion(domain_output.squeeze(), y.float())
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.log('test_loss', loss, on_step=True, logger=True)
        self.log_dict(self.test_metrics, on_step=True, logger=True)
        return loss
